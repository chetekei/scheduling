...
if search_policy:
    ...
    st.table(policy_results[['Insured ', 'Policy Number', 'ClaimType', 'Maturity Year', 'Claim Amount', 'Claim Amount', 'Installment']])

if search_name:
    name_results = df[df['Insured '].str.contains(search_name, case=False)]
    
    st.table(name_results[['Insured ', 'Policy Number','ClaimType', 'Maturity Year', 'Date Scheduled', 'Claim Amount', 'Installment']])
